    ...
    (tree: Tree, context: SchematicContext) =>
      options.subFolder && !options.onlyProject && options.createBase
        ? adjustBarrelIndex(
            'component',
            options,
            `libs/xplat/features/${componentSettings.featureName}/src/lib/base/${options.subFolder}/index.ts`,
            false,
            true
          )(tree, context)
        : noop()(tree, context),
    ...
    (tree: Tree, context: SchematicContext) =>
      !options.onlyProject && options.createBase
        ? adjustBarrelIndex(
            'component',
            options,
            `libs/xplat/features/${componentSettings.featureName}/src/lib/base/index.ts`,
            false,
            true
          )(tree, context)
        : noop()(tree, context),
