...
export default function(options: FeatureHelpers.Schema) {
  ...
  if (options.onlyProject) {
    ...
    // update index
    chains.push((tree: Tree, context: SchematicContext) =>
      FeatureHelpers.adjustBarrelIndex(options, `xplat/nativescript/features/index.ts`)(
        tree,
        context
      )
    );
    ...
    if (!options.onlyModule) {
      chains.push((tree: Tree, context: SchematicContext) =>
        FeatureHelpers.addFiles(options, 'nativescript', null, '_component')(tree, context)
      );
    }
  }
