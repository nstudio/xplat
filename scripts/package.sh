...
if [[ $XPLAT_VERSION == "*" ]]; then
    if [[ "$OSTYPE" == "darwin"* ]]; then
        if [[ $PACKAGE_VERSION =~ ^[0-9]+\.[0-9]+(\.[0-9]+)?$ ]]; then
          # override package version
          sed -E -i "" "s/\"@nstudio\/([^\"]+)\": \"\\*\"/\"@nstudio\/\1\": \"$PACKAGE_VERSION\"/" {schematics,angular,electron,electron-angular,ionic,ionic-angular,nativescript,nativescript-angular,web,web-angular,xplat}/package.json
        else 
