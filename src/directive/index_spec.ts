...
describe('directive schematic', () => {
  ...
  it('should create directive in libs by default for use across any platform and apps', () => {
    ...
    // component
    expect(files.indexOf('/libs/features/ui/directives/active-link.directive.ts')).toBeGreaterThanOrEqual(0);
  ...
  it('should create directive for specified projects only', () => {
    ...
    expect(files.indexOf('/libs/features/ui/directives/active-link.directive.ts')).toBe(-1);
    expect(files.indexOf('/xplat/nativescript/features/foo/directives/active-link.directive.ts')).toBe(-1);
    expect(files.indexOf('/xplat/web/features/foo/directives/active-link.directive.ts')).toBe(-1);

    // directive should be project specific
    expect(files.indexOf('/apps/nativescript-viewer/app/features/foo/directives/active-link.directive.ts')).toBeGreaterThanOrEqual(0);
    expect(files.indexOf('/apps/web-viewer/src/app/features/foo/directives/active-link.directive.ts')).toBeGreaterThanOrEqual(0);
    expect(files.indexOf('/apps/ionic-viewer/src/app/features/foo/directives/active-link.directive.ts')).toBeGreaterThanOrEqual(0);

    // file content
    let indexPath = '/apps/nativescript-viewer/app/features/foo/directives/index.ts';
