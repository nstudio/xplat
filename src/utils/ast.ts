 ...
 */
 ...
 /**
  * This has been slightly customized by nStudio for xplat
  */
...
export function addMethod(
  source: ts.SourceFile,
  modulePath: string,
  opts: { className: string; methodHeader: string; body: null | string }
): Change[] {
  const clazz = findClass(source, opts.className);
...
export function addToCollection(
  ...
  if (collection.hasTrailingComma || collection.length === 0) {
    ...
    return [new InsertChange(barrelIndexPath, collection.end, `,\n${insertSpaces}${symbolName}`)];
  }
}
...
function getCollection(source: ts.SourceFile): ts.NodeArray<ts.Expression> {
  const allCollections = findNodes(source, ts.SyntaxKind.ArrayLiteralExpression);
...
export function addGlobal(
  ...
  if (isExport) {
    ...
    if (allExports.length > 0) {
      ...
      return [
        new InsertChange(modulePath, lastExport.end, `\n${statement}\n`)
      ];
    } else {
    ...
    if (allImports.length > 0) {
      ...
      return [
        ...
        new InsertChange(modulePath, lastImport.end, `\n${statement}`)
      ];
    } else {
      return [new InsertChange(modulePath, 0, `${statement}`)];
    }
